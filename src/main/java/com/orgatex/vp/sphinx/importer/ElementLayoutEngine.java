...
public class ElementLayoutEngine {
  ...
  private void positionUseCasesInGrid(
      Map<String, Point> positions, List<String> useCaseIds, GridDimensions grid) {
  ...
  private void positionActorsAroundPerimeter(
      Map<String, Point> positions, List<String> actorIds, GridDimensions useCaseGrid) {
    if (actorIds.isEmpty()) {
      return;
    }
    ...
    int useCaseAreaWidth =
        useCaseGrid.columns * (ELEMENT_WIDTH + HORIZONTAL_SPACING) - HORIZONTAL_SPACING;
    int useCaseAreaHeight =
        useCaseGrid.rows * (ELEMENT_HEIGHT + VERTICAL_SPACING) - VERTICAL_SPACING;
  ...
  private Point calculateActorPosition(
      int actorIndex,
      int totalActors,
      int useCaseStartX,
      int useCaseStartY,
    ...
    if (position < useCaseHeight) {
      return new Point(useCaseStartX - 150, useCaseStartY + (int) position);
    }
    ...
    if (position < useCaseWidth) {
      return new Point(useCaseStartX + (int) position, useCaseStartY - 120);
    }
    ...
    // Right side
    if (position < useCaseHeight) {
      return new Point(useCaseStartX + useCaseWidth + 50, useCaseStartY + (int) position);
    }

    position -= useCaseHeight;

    // Bottom side
    return new Point(useCaseStartX + (int) position, useCaseStartY + useCaseHeight + 50);
  }
